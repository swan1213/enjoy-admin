...
interface BookingDetailsModalProps {
  ...
  onCompleted?: (id: string) => void; // Add callback for completion
}
...
export default function BookingDetailsModal({
  ...
  const handleMarkAsCompleted = async () => {
    ...
    try {
      await axios.patch(`${baseUrl}/bookings/admin/${selectedBooking.bookingId}/complete`, {}, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
        }
      });
      
      toast.success('Trajet marqué comme terminé avec succès');
      
      // Call the onCompleted callback if provided
      if (onCompleted) {
        onCompleted(selectedBooking.bookingId);
      }
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    ...
                    <span className={`px-2 py-1 rounded text-xs font-medium ${
                      ...
                      selectedBooking.status.toUpperCase() === 'PENDING' ? 'bg-yellow-100 text-yellow-800' : 
                      selectedBooking.status.toUpperCase() === 'CANCELLED' ? 'bg-red-100 text-red-800' :
                ...
                {selectedBooking.status.toUpperCase() !== "COMPLETED" && 
                 selectedBooking.status.toUpperCase() !== "CANCELLED" && (
                  <Button
                    ...
                    className="bg-blue-600 hover:bg-blue-700 mt-2 flex items-center gap-2"
                  >
                    <CheckCircle className="h-4 w-4" />
                    {isMarkingComplete ? 'Traitement...' : 'Marquer comme terminé'}
                  </Button>
                )}
